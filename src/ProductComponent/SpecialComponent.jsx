...
const SpecialComponent = () => {
  ...
  return (
    <>
      <div className="container text-center border-dashed border-t-2 pt-3 border-cyan-300">
        <h3 className=" text-red-400 font-bold text-2xl">محصولات ویژه</h3>
        <div className="mx-10 mt-7 grid grid-cols-5 space-x-5 justify-items-center">
          {specialProduct.map((item) => (
            <div
              key={item.id}
              onMouseEnter={(e) => show(e, item)}
              onMouseLeave={unshow}
              className="w-[180pt] rounded-xl hover:shadow-2xl hover:shadow-slate-800  ring-2"
            >
